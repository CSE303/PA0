...
# file, and then links .o fiels together to produce an executable. This version
# is modified slightly, so that it can make multiple exdcutables. The breakdown
...
# rebuilt, and nothing more. You should'nt need to type 'make clean' very often
#
...
TARGETS = echo int_ops # TODO: put your file names here, *without a file extension*
...
# This line ensures that the above folder will be created before any compiing happens
...
# Generate the names of the dependecy files that g++ will generate, so that we
...
# optimizations, and generate dependecny information on-the-fly
CXX = g++
LD = g++
...
# NB: On the next oine, Docker is fine with '-lcrypto', but that doesn't work on
# SunLab. The more explicit '-l:libcrypto.so.3' works on Docker and SunLab
LDFLAGS = -m$(BITS) -lpthread -l:libcrypto.so.3 -ldl $(LDEXTRA)

# Build 'all' by defualt, and don't clober.o files after each build
.DEFAULT_GOAL = all
.PRECIOUS: $(ALLOFILES)
.PHONY: all clean
...
# Rules for building exectuables... we assume an executable uses *all* of the 
# common OFILES
...
# Include any dependencies we generate previously
-include $(DFILES)